...
// Insert/update a full row (rec must supply every column) per mode, keeping secondary indexes in sync; false with *err
// set if rec is malformed or mode's existence requirement isn't met.
bool db_update(KV* kv, const TableDef& tdef, const Record& rec, UpdateMode mode, std::string* err);

// Point delete by primary key: `rec` must supply exactly the pk columns. Also removes the row's index keys.
